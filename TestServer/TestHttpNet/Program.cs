...
namespace TestHttpNet
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            HttpQuery http_query_get = new HttpQuery();
            ...
            net.Request(HttpNetRequestType.GET, "https://partner.steam-api.com/ISteamMicroTxn/GetUserInfo/v2", http_query_get, out response);
            Console.WriteLine(response);

            HttpQuery http_query_post = new HttpQuery();
            http_query_post.time_out = 10000;
            http_query_post.AddParam("id", "101");
            http_query_post.AddParam("name", "Alex");

            net.Request(HttpNetRequestType.POST, "https://httpbin.org/post", http_query_post, out response);
            Console.WriteLine(response);
